...
> **对应课件**：《第3章 移植U-Boot》3.5 节，Slide 43-49
>
...
![装 gdisk 并验证](./05_实验四_SD卡分区烧写与首次启动.assets/01_装gdisk.png)
...
![编译产物仍在](./05_实验四_SD卡分区烧写与首次启动.assets/02_编译产物.png)
...
![基线 lsblk](./05_实验四_SD卡分区烧写与首次启动.assets/03_基线lsblk.png)
...
![取下 microSD 卡](./05_实验四_SD卡分区烧写与首次启动.assets/04_取出SD卡.jpeg)
> 图：在开发板卡槽处轻轻往里推动，卡即弹出（板子先断电；卡槽就在按键旁边）。

![插入 USB 读卡器](./05_实验四_SD卡分区烧写与首次启动.assets/05_插读卡器.jpeg)
...
![USB 设备连接到虚拟机](./05_实验四_SD卡分区烧写与首次启动.assets/06_USB提示框.png)
...
![插入读卡器后的设备节点](./05_实验四_SD卡分区烧写与首次启动.assets/07_设备节点.png)
...
![旧分区表](./05_实验四_SD卡分区烧写与首次启动.assets/08_旧分区表.png)
...
![分区操作四步实录](./05_实验四_SD卡分区烧写与首次启动.assets/09_分区四步实录.png)
...
![sgdisk -Z 清场实录](./05_实验四_SD卡分区烧写与首次启动.assets/10_sgdisk清场.png)
...
![清空后的复查](./05_实验四_SD卡分区烧写与首次启动.assets/11_清空复查.png)
...
![重建五个分区](./05_实验四_SD卡分区烧写与首次启动.assets/12_重建五分区.png)
...
![三条 dd 写入成功](./05_实验四_SD卡分区烧写与首次启动.assets/13_三条dd写入.png)
...
![断开读卡器](./05_实验四_SD卡分区烧写与首次启动.assets/14_SD卸载.png)
...
2. 把启动**拨码开关置为 `101`**（从 SD 卡启动）——板上是三位拨码，丝印与 `BOOT2 / BOOT1 / BOOT0` 一一对应，`101` 即按“ON、OFF、ON”拨（1 = ON 侧，0 = OFF 侧）：
   ...
   ![启动拨码 SW1](./05_实验四_SD卡分区烧写与首次启动.assets/15_启动拨码.jpeg)
   > 图：FS-MP1A 的启动拨码（SW1）——三位自上而下对应 `BOOT2 / BOOT1 / BOOT0`，ON 侧为 1、OFF 侧为 0；从 SD 卡启动就拨成 `101`。

   ![拨码四种配置](./05_实验四_SD卡分区烧写与首次启动.assets/16_拨码四种配置.jpeg)
...
![复位循环静帧](./05_实验四_SD卡分区烧写与首次启动.assets/17_复位循环静帧.png)
> 图：COM11 串口实录的静帧——同一段日志一遍遍重复，这就是“复位循环”的样子（即下方动图的一帧）。

![复位循环动图](./05_实验四_SD卡分区烧写与首次启动.assets/18_复位循环动图.gif)
> 图：复位循环现场实录（动图）——每轮报错后 `resetting ...` 从头再来，无限重复；这个循环正是实验五（F-1）要终结的。
