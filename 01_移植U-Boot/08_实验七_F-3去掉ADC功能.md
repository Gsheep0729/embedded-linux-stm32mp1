...
**实际执行结果**（2026-09-18 补记）：`cp` 与提交均已完成——这步当时没单独截图，完成与否由下一次构建的版本串反证：实验八（F-4）上电日志里 SPL 与 U-Boot 的版本串为 `2020.01-stm32mp-r1-g1ac3a506-dirty`，其中哈希 `g1ac3a506` 正是本次提交入库后的 HEAD（提交前 HEAD 是 F-2 的 `g8de188df`，实验七 17:04 构建的版本串可证），`-dirty` 后缀则是彼时工作区里 F-4 的设备树改动。版本串的哈希接力——实验四 `g3f0216e7`（初始 7 条提交）→ 实验六 `g2224655f`（F-1）→ 实验七 `g8de188df`（F-2）→ 实验八 `g1ac3a506`（F-3）——就是每次"验证通过即提交"留下的收条。
