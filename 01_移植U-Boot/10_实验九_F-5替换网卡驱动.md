...
驱动源在 Windows 的资料包里：`u-boot-网卡-MAE0621A驱动.rar`。在 Windows 上解压（右键解压即可），应得到三个 `.c` 文件——`phy.c`、`maxio.c`、`dwc_eth_qos.c`（若压缩包里还套着目录，取到文件本身即可）。

把三个文件拷进共享文件夹（本文假设放在 `~/Desktop/LINUX-gy/Test2/`，与前面实验一致；放别处就改下面命令的路径），虚拟机里确认能见到：

```bash
ls -l ~/Desktop/LINUX-gy/Test2/{phy.c,maxio.c,dwc_eth_qos.c}
```
...
```bash
cp ~/Desktop/LINUX-gy/Test2/phy.c       drivers/net/phy/phy.c       # 覆盖同名文件
cp ~/Desktop/LINUX-gy/Test2/maxio.c     drivers/net/phy/maxio.c     # 新增
cp ~/Desktop/LINUX-gy/Test2/dwc_eth_qos.c drivers/net/dwc_eth_qos.c # 覆盖同名文件
```
...
git status --short                       # 应显示 M drivers/net/dwc_eth_qos.c、M drivers/net/phy/phy.c
```
...
- 三个驱动文件各就各位：`drivers/net/phy/maxio.c`（新增）、`drivers/net/phy/phy.c` 与 `drivers/net/dwc_eth_qos.c`（覆盖为厂商改造版）
- `include/phy.h`、`drivers/net/phy/Makefile`、`drivers/net/phy/Kconfig` 三处配套修改齐全，menuconfig 已勾选 `CONFIG_PHY_MAXIO=y` 并同步进 defconfig
- 重新编译成功（`maxio.o` 生成），三个镜像已重新烧写到 sdb1 / sdb2 / sdb3
- 启动日志不再出现 `EQOS_DMA_MODE_SWR stuck`
- 板子 `ping` 通 PC（`host ... is alive`）
- 已完成 defconfig 同步与 git 提交（7 个文件）
