...
> **抢不赢倒计时，就把窗口调宽**：停进命令行后的第一件事，不如先把 `bootdelay`（autoboot 倒计时秒数）设成 5 再干正事——这台板子上实测它当前是 0（`Hit any key to stop autoboot:  0` 那个 0 就是它），归零只在眨眼之间：
>
> ```
> setenv bootdelay 5
> saveenv
> ```
>
> 存盘后 `reset` 一次即可看到倒计时从 5 数起，往后每次拦停都不用抢。这条改动一直受益到后续所有实验（实验十一的 trusted 版、第 4 章整章都在练拦停）。两种写法等价——`setenv bootdelay 5` + `saveenv`，或 `env set bootdelay 5` + `env save`（`env` 家族的子命令与顶层老名字是同一段代码的两个门牌，`help` 列表里都在）。万一敲下去报 `Unknown command 'xxx'`，先别怀疑命令没编进来：终端粘贴要用右键、别从 PDF 或网页直接拷——零宽字符、全角空格会跟进命令词里，终端不回显，U-Boot 却把它当成命令名的一部分，手敲一遍是最快的判别。
