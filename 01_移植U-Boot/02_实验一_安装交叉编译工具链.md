...
> **对应课件**：《第3章 移植U-Boot》3.4 节，Slide 28-31
>
> **系列说明**：本系列基于华清远见 FS-MP1A（STM32MP157A）开发板，对应课件《第3章 移植U-Boot》。本文覆盖 Slide 28-31。所有操作在 VMware + Ubuntu 20.04 虚拟机中完成。
...
| 项目 | 实际值 |
|---|---|
| 虚拟机 | VMware + Ubuntu 20.04（focal），主机名 `cnu-virtual-machine`，用户 `cnu` |
| 网络 | 桥接网卡 |
...
**做什么**：在 Windows 上用 MobaXterm 新建一个 Serial（串口）会话，连接板载 ST-Link 的虚拟串口（COM 口），波特率设为 **115200**（ST 方案出厂默认值），给开发板上电，观察输出。

**为什么第一步不是装软件，而是看串口？**

因为接下来的一系列实验里，板子上发生了什么、报了什么错，**全部要靠串口告诉我们**。串口就是整个移植过程的"眼睛"——必须最先确认它是通的。否则以后编译、烧写出了问题，你连"板子到底是没跑起来还是跑了没输出"都无从判断。

在 Windows 上用 MobaXterm 新建 Serial 会话（菜单 Session → Serial，Speed (bps) 选 **115200**），配置界面如下：

![MobaXterm 串口会话设置](./02_实验一_安装交叉编译工具链.assets/01_串口会话设置.png)  
> 图：MobaXterm 新建 Serial 会话的设置界面——Session → Serial，Serial port 选 `Choose at session start`（连接时再选板子的 COM 口），Speed (bps) 选 115200。

给开发板上电后，成功抓到出厂固件的启动日志：
...
**这段日志值得精读**——它按时间顺序讲了一个完整的启动故事，而且藏着后面两个实验要用到的线索：
...
1. **`Enter target directory for SDK (default: /opt/st/stm32mp1/...):`** ——"你想装到哪？"直接回车即可。默认的 `/opt` 是 Linux 文件系统层次标准（FHS）里约定的**第三方大型软件**安装位置，ST 的工具链放这里最合适。
   > **注意**：这里只能**直接回车**或输入**绝对路径**——输相对路径的话，SDK 会装进当前目录下的一个子文件夹里，位置不受控。另外切记不要装到虚拟机共享文件夹里：共享文件夹**不支持 Linux 符号链接**，而工具链内部大量使用符号链接，装在那里后面编译必踩坑。
...
（直接 source 完整路径完全等价。**软链接建过一次就一直在**：重复执行 `ln -s` 会报"文件已存在"，无害——链接已在，直接 `source /stm32env` 即可。下方实际输出记录的是当初用完整路径激活的终端，效果与 `source /stm32env` 一致；本系列后续实验的激活命令统一写 `source /stm32env`。）
...
3. **工具链必须装在虚拟机本地磁盘**（`/opt/st`）——不要装到共享文件夹里，共享文件夹不支持 Linux 符号链接。

## 五、实验结果

| 检查项 | 结果 |
|---|---|
| 串口验证（MobaXterm，115200） | 抓到出厂固件启动日志 |
| git 安装并完成全局配置 | 完成 |
