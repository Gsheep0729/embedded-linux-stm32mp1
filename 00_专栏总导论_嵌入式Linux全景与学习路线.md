...
>
> **软硬件平台**：华清远见 FS-MP1A 开发板（ST STM32MP157AAA3）+ VirtualBox 虚拟机（Ubuntu 20.04）+ ST 官方 SDK 交叉编译工具链。全套内容基于板卡实际操作，文中所有终端输出、报错、截图均为真实实验记录。
...
|---|---|
| 专栏总导论（本篇） | ✅ |
| U-Boot 实验导论：看懂 U-Boot，再谈移植 | ✅ |
| 实验一 安装交叉编译工具链 | ✅ |
| 实验二 获取源码并打 ST 补丁 | ✅ |
| 实验三 basic 版配置与首次编译 | ✅ |
| 实验四 SD 卡分区烧写与首次启动 | ✍️ 写作中 |
| F-1~F-6 驱动修复系列 / trusted 版移植 | ⏳ 实验中 |

**第二部分：内核与根文件系统（第 4~6 章）** ⏳ 计划中

**第三部分：驱动开发（第 7~9 章）** ⏳ 计划中

**第四部分：应用开发（第 10 章）** ⏳ 计划中
