...
![FS-MP1A开发板](./00_专栏总导论_嵌入式Linux全景与学习路线.assets/slide002_01.jpg)  
...
![嵌入式Linux启动流程](./00_专栏总导论_嵌入式Linux全景与学习路线.assets/slide003_02.png)  
...
**第一部分：U-Boot 移植（第 3 章）**

| 篇目 | 内容 |
|---|---|
| 专栏总导论（本篇） | 全景与学习路线：启动链条、十章地图 |
| 实验导论：看懂 U-Boot，再谈移植 | U-Boot 是什么、源码工程目录速览 |
| 实验一 安装交叉编译工具链 | 装好、激活、验证 ST 的 SDK |
| 实验二 获取 U-Boot 源码并打 ST 补丁 | 官方源码 + 6 个 ST 补丁 + git 工作流 |
| 实验三 basic 版 U-Boot 配置与首次编译 | 复制 defconfig 与设备树、注册 dts、首次编译 |
| 实验四 SD 卡分区烧写与首次启动 | 分区、dd 烧写、拨码 101 首次上电 |
| F-1~F-6 驱动修复 | 按串口报错逐个修：电源 / SD 卡检测 / ADC / 显示 / 网卡 / eMMC |
| trusted 版移植 | 换 TF-A 引导链，把 basic 版的成果搬过去 |

**第二～四部分（第 4~10 章）**：Linux 内核与根文件系统（U-Boot 命令行 → 内核移植 → BusyBox / Buildroot 构建根文件系统）、驱动开发（字符设备 → GPIO → 设备树）、应用开发（Qt 图形界面）。
