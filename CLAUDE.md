...
- **文档开头标注课件 Slide 范围（用户指定，2026-09-18）**：每篇实验文档标题下「系列说明」引用块的**首行**固定写 `> **对应课件**：《第N章 …》X.Y 节，Slide A-B`，便于对照课件原文快速定位；新文档照此格式补齐。当前映射：导论 3.1~3.3 节 Slide 1-26、实验一 3.4 节 28-31、实验二 3.4 节 32-34、实验三 3.5 节 35-42、实验四 3.5 节 43-49、实验五 3.5 节 50-59、实验六 3.5 节 60-65、实验七 3.5 节 66-68（第3章共 87 页：3.1=1-5、3.2=6-14、3.3=15-26、3.4=27-34、3.5=35-81、3.6=82-87）。
...
- U-Boot 源码（已打 6 个 ST 补丁，WORKING 分支 8 条提交 = 初始 7 条 + F-1，F-2 的提交待做）：`~/Desktop/LINUX-gy/Test2/stm32mp1-openstlinux-5.4-dunfell-mp1-20-06-24/sources/arm-ostl-linux-gnueabi/u-boot-stm32mp-2020.01-r0/u-boot-stm32mp-2020.01`。
- basic 版编译：`make -j2 all DEVICE_TREE=stm32mp157a-fsmp1a` → 产物 `u-boot-spl.stm32`（FSBL）+ `u-boot.img`（SSBL）。
- 串口：Windows 上用 MobaXterm 的 Serial 会话连板载 ST-Link 虚拟串口（实测为 `COM11`），115200；板子从 SD 卡跑我们自己的 basic 版 U-Boot，F-2 之后已能停在 `STM32MP>` 命令行（剩 ADC/网卡/bootm 找不到内核等报错，F-3 起逐个处理）。
...
- 实验七指导已写好（2026-09-18）：`01_移植U-Boot/08_实验七_F-3去掉ADC功能.md`（Slide 66-68 → 步骤 1~7：menuconfig 关 `Command line interface → Device access commands → adc` 与 `Device Drivers → Enable ADC drivers using Driver Model` 两项，即 CONFIG_CMD_ADC + CONFIG_ADC；收尾 `cp .config configs/stm32mp15_fsmp1a_basic_defconfig` + git 提交；步骤 2~7 留"实际执行结果：待补充"；assets 已放课件图 4 张 01~04，用户实测截图从 05 续编），待用户执行后回填。
- 下一步：实验七 = F-3 去掉 ADC（Slide 66-68）→ F-4 关 LTDC（Slide 69 起，`stm32mp15xx-fsmp1x.dtsi` 的 `&ltdc` 改 `status = "disabled"` + pinctrl 收敛）→ F-5 国产网卡 MAE0621A（Slide 70 起）→ F-6 eMMC；最后移植 trusted 版。
