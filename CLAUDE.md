...
- **文档开头标注课件 Slide 范围（用户指定，2026-09-18）**：每篇实验文档标题下「系列说明」引用块的**首行**固定写 `> **对应课件**：《第N章 …》X.Y 节，Slide A-B`，便于对照课件原文快速定位；新文档照此格式补齐。当前映射：导论 3.1~3.3 节 Slide 1-26、实验一 3.4 节 28-31、实验二 3.4 节 32-34、实验三 3.5 节 35-42、实验四 3.5 节 43-49、实验五 3.5 节 50-59、实验六 3.5 节 60-65、实验七 3.5 节 66-68、实验八 3.5 节 69（第3章共 87 页：3.1=1-5、3.2=6-14、3.3=15-26、3.4=27-34、3.5=35-81、3.6=82-87）。
...
- U-Boot 源码（已打 6 个 ST 补丁，WORKING 分支 9 条提交 = 初始 7 条 + F-1 + F-2，F-2 提交 = `g8de188df`；F-3 的提交 = 实验七步骤 7，文档尚未回填）：`~/Desktop/LINUX-gy/Test2/stm32mp1-openstlinux-5.4-dunfell-mp1-20-06-24/sources/arm-ostl-linux-gnueabi/u-boot-stm32mp-2020.01-r0/u-boot-stm32mp-2020.01`。
...
- 已完成（2026-09-18）：实验七 Slide 66-68（F-3 去掉 ADC）——指导 `01_移植U-Boot/08_实验七_F-3去掉ADC功能.md` 步骤 1~6 已回填实测与 4 张截图（05 menuconfig 后自检 not set 恰两行、06 编译收尾+lsblk+三条 dd、07 串口动图、08 完整日志；assets 02/03 两张课件图内容与文件名原本对调——02 实为 menuconfig 界面、03 实为两条路径文字总览——已换正并补图注）。实测串口（17:04 构建，版本串 `g8de188df` **无 `-dirty`** = `.config` 不入库、改配置不弄脏工作区，哈希即 F-2 提交 → 实验六步骤 7 已完成）：三条 ADC 报错（含 `vrefbuf timed out`）全部消失、`Err: serial` 直达 `Net:`，EQOS×2 与 bootm 报错照旧，停在 `STM32MP>`。**步骤 7（`cp .config configs/stm32mp15_fsmp1a_basic_defconfig` + git 提交）尚未回填**——F-3 的 defconfig 提交是否已做待用户确认。
- 实验八指导已写好（2026-09-18）：`01_移植U-Boot/09_实验八_F-4关闭LTDC显示.md`（Slide 69 单页 → 步骤 1~7，全部留"实际执行结果：待补充"；assets 01 = slide069_58.png 课件修改结果图）。配方：`stm32mp15xx-fsmp1x.dtsi` 的 `&ltdc` **只改 `status = "okay" → "disabled"` 一行**，`port` 段保留勿删（原因见下方踩坑记录 F-4 锚点）；验证重心在源码/dtb 层（串口对 F-4 无感，里程碑 = 输出与实验七完全一致）。下一步：用户执行实验八 → 实验九 F-5 网卡 MAE0621A（Slide 70 起）→ F-6 eMMC；最后 trusted 版。
