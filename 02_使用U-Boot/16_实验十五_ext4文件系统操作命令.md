...
| 网络 | Ubuntu 侧 `tftpd-hpa` 运行中（实验十三搭的那台：`/home/cnu/tftpboot`，`serverip 192.168.0.100` 已存盘），里面有 `uImage`（7,546,640 字节） |
...
实验十七的 eMMC 启动路径需要"内核 + 设备树"一对文件。若步骤 1 清单里已有配套的 fsmp1a 系 dtb 可跳过；没有的话，把 `官方系统内核和设备树.zip` 里那个 `stm32mp157a-fsmp1a-mipi050.dtb`（71,805 字节）按实验十三的路径送进服务器——解压 → 放共享目录 → `sudo cp` 到 `/home/cnu/tftpboot/` → **`chmod 644`**（共享目录 `cp` 过来默认是 700，板子拉不动，见实验十三坑 7），然后：
...
- `ext4ls` 列出 eMMC ext4 分区内容（步骤 1，待实测）
- `ext4load` 读 uImage 进 DRAM，回显字节数与清单一致（步骤 2，待实测）
- `tftp c0000000 uImage` + `ext4write ... /test_uImage 0x732710` 全链路走通（步骤 3~4，待实测）
- `ext4ls` 复验看到 `7546640 test_uImage`（步骤 5，待实测）
- （选做）dtb 已写入分区，实验十七的 eMMC 启动料已备齐（步骤 6，待实测）
