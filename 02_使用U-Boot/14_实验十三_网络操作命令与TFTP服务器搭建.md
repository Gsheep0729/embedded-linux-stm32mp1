...
**TFTP 是什么，为什么嵌入式圈都用它**：它是文件传输协议里最简的一个——跑在 UDP 69 端口上，没有账号体系、没有目录浏览，客户端说"给我这个文件名"，服务器就把根目录里同名文件送过来，一次一问一答。正因为简单，**U-Boot 里内置了客户端，服务器端一条 `apt install` 就完事**，改一次内核传一次文件不用拔卡；代价是没有安全机制，所以只在实验室这根网线内部用。
