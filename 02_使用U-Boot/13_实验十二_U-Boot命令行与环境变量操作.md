...
|---|---|---|
| `bootdelay 5` + `saveenv`（开工自检/步骤 4） | 第 4 章所有实验的每一次拦停（实验十三~十七全靠它从容进命令行） | 倒计时回到 0，每次上电都在抢按键，漏按一次就错过 |
| `help` / `?`、命令缩写、Tab 补全、数值一律十六进制（步骤 2） | 全系列每一天：`tftp c2000000 uImage`、`mmc read c0000000 400 10` 这些裸十六进制地址都靠这条约定 | 看到不带 `0x` 的地址当成十进制，读错一个数量级 |
| `bdinfo` / `printenv` / `version` 三条查询（步骤 3） | 排障第一手：`bdinfo` 的 DRAM 起止与 `relocaddr`、`printenv` 核对网络四件套、`version` 反查卡上镜像是哪次构建 | 板子状态说不清，出问题只能瞎猜 |
| **"改完必须 saveenv"纪律**（步骤 4~6 的对照实验） | 实验十三的 `serverip`、实验十六的 `bootcmd`、实验十七的 `mybootnet`/`mybootemmc`——全靠它落盘 | 变量只在内存里，一次复位就"消失"，误以为板子坏了 |
| `printenv` 网络四件套复核（步骤 7） | 实验十三整篇的网络前提 | 出厂 `serverip=192.168.1.1` 没被覆盖的话，实验十三的 tftp 会静默去敲错误地址干等超时 |
| 环境存于 SD 卡（`Writing to MMC(0)`） | 换卡/重分区后环境会丢，要按恢复手册重设 | 换了卡以为变量还在，一上电全变回出厂值 |

本篇**不需要任何新资料**：不碰源码、不编译、不烧写，全部动作发生在串口的 `STM32MP>` 后面。
